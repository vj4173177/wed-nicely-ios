...
final class APIManager {
    ...
    func request<T: Codable>(
        ...
        print("Requested URL : \(String(describing: type.requestUrl))")
        guard let url = type.requestUrl else {
            completion(.failure(.invalidURL))
            return
        }

        var request = URLRequest(url: url)
        request.httpMethod = type.requestMethod.rawValue

        if let parameters = type.requestBody {
            request.httpBody = try? JSONEncoder().encode(parameters)
        }

        request.allHTTPHeaderFields = type.requestHeader
